...
const formatCurrency = (value: string | number | null | undefined, currency = DEFAULT_CURRENCY) =>
  `${amountNumber(value).toFixed(2)} ${currency || DEFAULT_CURRENCY}`
...
export default function HrPage() {
  ...
  const getEmployeeContractCurrency = (employeeId: string) => {
    ...
    const latestContract = contracts.filter((c) => c.employee === employeeId).sort((a, b) => b.start_date.localeCompare(a.start_date))[0]
    return latestContract?.currency || DEFAULT_CURRENCY
  }
  ...
  return (
    <div className="space-y-6 p-4 md:p-8">
      ...
      <Tabs value={activeTab} onValueChange={(value) => setActiveTab(value as HrTab)} className="space-y-4">
                ...
                <TableBody>
                  {filteredContracts.map((c) => (
                    <TableRow key={c.id}>
                      ...
                      <TableCell>{formatCurrency(c.salary, c.currency || DEFAULT_CURRENCY)}</TableCell>
      ...
      <Dialog open={contractDialogOpen} onOpenChange={setContractDialogOpen}>
        <DialogContent className="sm:max-w-3xl">
          ...
          <form className="grid gap-3" onSubmit={submitContract}>
            ...
            <div className="grid gap-3 md:grid-cols-2">
              <div className="space-y-2"><Label>Salaire ({contractForm.currency || DEFAULT_CURRENCY})<RequiredMark /></Label><Input placeholder="Ex: 250000" type="number" value={contractForm.salary} onChange={(e) => setContractForm((p) => ({ ...p, salary: e.target.value }))} required /></div>
      ...
      <Dialog open={payrollRuleDialogOpen} onOpenChange={setPayrollRuleDialogOpen}>
        <DialogContent className="sm:max-w-2xl">
          ...
          <form className="grid gap-3" onSubmit={submitRule}>
            ...
            <div className="space-y-2"><Label>Montant ({DEFAULT_CURRENCY})<RequiredMark /></Label><Input placeholder="Ex: 15000" type="number" value={payrollRuleForm.default_amount} onChange={(e) => setPayrollRuleForm((p) => ({ ...p, default_amount: e.target.value }))} required /></div>
